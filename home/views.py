...
from .multi_trame_server_uncommented import ServerManager
...
@csrf_exempt
def start_new_server(request):
    if request.method == 'POST':
        server_manager.start_new_server()
        return JsonResponse({'server_list': server_manager.get_server_list()})
...
def visualization_server_manager(request):
    if request.method == 'POST':
        # Expecting a JSON body, so handle potential issues with body parsing
        try:
        ...
        action = data.get('action')  # Safely get the 'action' from JSON
        port = data.get('port')  # Safely get the 'port' from JSON (if stopping a server)

        # Handle 'start' action
        if action == 'start':
            server_manager.start_new_server()
            return JsonResponse({'status': 'success', 'server_list': server_manager.get_server_list()})
        # Handle 'stop' action
        elif action == 'stop' and port:
            server_manager.stop_server(port)
            return JsonResponse({'status': 'success', 'server_list': server_manager.get_server_list()})
        else:
    ...
    # For GET requests, return the server list in the template context
    server_list = server_manager.get_server_list()
    return render(request, 'visualization_server_manager.html', {'server_list': server_list})

# AJAX for getting the server list dynamically
def get_server_list(request):
    if request.method == 'GET':
        return JsonResponse({'server_list': server_manager.get_server_list()})

# Function to initialize and serve the Trame app
def histogram_view(request):
    ...
    # Initialize VTK table and render
    ...
    # Layout for the Trame app
    with SinglePageLayout(server) as layout:
        with layout.content:
            with vuetify.VContainer(fluid=True, classes="pa-0 fill-height"):
                vuetify.VSlider(v_model=("num_bins", 20), min=10, max=100, step=1, label="Number of Bins")
    ...
    return render(request, 'trame_histogram.html', {})