...
def test_histogram_render_time():
    with sync_playwright() as p:
        ...
        page.wait_for_selector(".v-main", timeout=5000)
            ...
            try:
                ...
                result_handle = page.wait_for_function(
                    ...
                    () => {
                        ...
                        const STABLE_THRESHOLD = 1;  // Number of consecutive stable checks required

                        window.__check_count += 1;

                        const vtkImgBlob = document.querySelector('.v-main img');
                        if (!vtkImgBlob) return false;

                        const previousState = window.__previous_vtk_state || null;
                        const currentState = {
                            innerHTML: vtkImgBlob.innerHTML,
                            attributes: Array.from(vtkImgBlob.attributes).reduce((acc, attr) => {
                        ...
                        window.__previous_vtk_state = currentState;
