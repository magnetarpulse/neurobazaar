...
class WslinkHandler(object):
    ...
    async def onCompleteMessage(self, rpc, client_id):
        ...
        try:
            ...
            if client_id not in self.connections or self.connections[client_id].closed:
                logger.debug(f"Client {client_id} disconnected during RPC call, skipping response")
                return
