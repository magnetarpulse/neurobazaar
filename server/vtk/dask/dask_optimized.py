...
class HistogramApp:    
    ...
    def compute_histogram_data_with_dask(self, dask_data, bins):
        ...
        start_time_to_convert_to_numpy = time.time()
        self.hist = self.convert_to_numpy(self.hist)
        end_time_to_convert_to_numpy = time.time()

        print(f"Calculating the histogram using took {end_time_to_calculate_histogram - start_time_to_calculate_histogram} seconds")
        print(f"Converting the histogram to a NumPy array took {end_time_to_convert_to_numpy - start_time_to_convert_to_numpy} seconds")
    
    # ---------------------------------------------------------------------------------------------
    # Method using Dask compute method to convert Dask object to NumPy array
    # ---------------------------------------------------------------------------------------------

    def convert_to_numpy(self, dask_object):
        result = dask_object.compute(scheduler='threads', num_workers=86)
        return result
