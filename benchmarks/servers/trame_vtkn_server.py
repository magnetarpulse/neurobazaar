...
class VtkApp:  
    def __init__(self, data_size=1_000, ui_type = "slider ", port: int = 8080, remote_rendering: bool = True):
        ...
        if isinstance(data_size, int):
            ...
            self.data = np.random.normal(size=data_size)
    ...
    def _update_histogram(self):
        ...
        print(f"Bin size: {self.state.bins}")
    ...
    def _compute_hist(self, numpy_data, bins):
        """Compute histogram values using NumPy"""
        print(f"Bin size: {self.state.bins}")
        print(f"Type bins: {type(bins)}")
        if not isinstance(bins, int):
            print("\033[91mWARNING: Invalid bins value: {bins}\033[0m")
            print("\033[94mINFO: Trying to convert bins to integer\033[0m")
            bins = int(bins)
            if not isinstance(bins, int):
                print(f"\033[91mERROR: Could not convert. Bin is currently of type {type(bins).__name__}\033[0m")
                return None, None
            print("\033[92mSuccessfully converted bins to integer\033[0m")

        try:
            if not isinstance(bins, int) or bins <= 0:
                raise ValueError(f"Invalid bins value: {bins}")
            hist, bin_edges = np.histogram(numpy_data, bins=bins)
...
if __name__ == "__main__":
    ...
    logging_statistics = ReLogger(log_file="vtkn_server_read_data.log", create_dir="results")
    ...
    # vtk_app= VtkApp(data_size=1_000_000_000, ui_type="input", remote_rendering=True)
    vtk_app.start_now()
    vtk_app.stop()
