...
watch(
    figureData,
    (data) => {
        const container = document.getElementById("histogram_plotly");

        if (data && container) {
            let startTime = performance.now();
            
            Plotly.newPlot('histogram_plotly', JSON.parse(data));
            
            let endTime = performance.now();
            let timeDiff = endTime - startTime;

            console.log(`Time taken by Plotly.newPlot: ${timeDiff} ms`);
        } else {
