...
class BaseHistogramApp:    
    ...
    def update_histogram(self, bins):
        ...
        # self.compute_histogram_data_with_dask(self.np_data, bins) # Use for dask distributed
        ...
        with SinglePageLayout(self.server) as layout:
            ...
            with layout.content:
                with vuetify.VContainer(
                    fluid=True,
                    classes="pa-0 fill-height",  # Full-height to fill the content area
                ):
...
class ServerManager:
    ...
    def get_server_list(self):
        # Return a list of currently active servers
        return [
            {
                'port': server['port'],
