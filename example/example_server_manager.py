...
from example_generic_histogram import GenericHistogramApp
from example_standalone_histogram import BasicHistogramApp
from simple_server import main_async

# Import Django models
import django
os.environ.setdefault('DJANGO_SETTINGS_MODULE', 'neurobazaar.settings')
django.setup()
from home.models import ServerInstance
...
class ServerManager:
    ...
    def _load_existing_servers(self):
        try:
            running_servers = ServerInstance.objects.filter(is_running=True)
            for server in running_servers:
                if server.server_type == 'basic':
                    self.control_state.basic_server_list.append({
                        'port': server.port,
                        'status': 'Running'
                    })
                elif server.server_type == 'general':
                    self.control_state.general_server_list.append({
                        'port': server.port,
                        'status': 'Running'
                    })
                elif server.server_type == 'analyzer':
    ...
    def start_new_basic_server(self):
        ...
        command = [
            "python",
            "example/example_server_manager.py",  
            "--launch_basic_server",
            str(self.next_port)
        ]
    ...
    def start_new_simple_server(self):
        print("Starting new simple server")
    ...
    def stop_basic_server(self, port):
        ...
        try:
            ...
            ServerInstance.objects.filter(port=port, server_type='basic').delete()
    ...
    def stop_general_server(self, port):
        ...
        try:
            ...
            # Delete from database
            ServerInstance.objects.filter(port=port, server_type='general').delete()
    ...
    def stop_simple_server(self, port):
        ...
        try:
            ...
            # Delete from database
            ServerInstance.objects.filter(port=port, server_type='analyzer').delete()
    ...
    def trigger_stop_simple_server(self, port):
        print("Stopping simple server with port:", port)
        ...
        with SinglePageLayout(self.servers_manager) as layout:
            ...
            with layout.content:
                ...
                with vuetify.VRow():
                    with vuetify.VCol(cols="auto"):
                        vuetify.VBtn(
                            "Start New Neurobazaar Simple Server",
                ...
                with vuetify.VCard():
                    vuetify.VCardText("Active Simple Server(s):")
                    with vuetify.VList(shaped=True):
                        with vuetify.VListItem(v_for="(server, idx) in analyzer_server_list", key="idx"):
                            ...
                            with vuetify.VListItemContent():
                                vuetify.VListItemTitle("{{ server.port }} - {{ server.status }}")
                            with vuetify.VBtn(icon=True, click="trigger('trigger_stop_simple_server', [server.port.toString()])"):
                                vuetify.VIcon("mdi-stop", color=("level_to_color[server.status]",))
