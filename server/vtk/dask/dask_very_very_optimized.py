...
class HistogramApp:    
    def __init__(self, np_data=None):
        ...
        self.np_data = np_data if np_data is not None else 0
    ...
    def compute_histogram_data_with_dask(self, dask_data, bins):
        ...
        hist_result, bin_edges = self.convert_agghistogram_to_numpy(dask_hist)
        self.hist = hist_result.view()
        self.bin_edges = bin_edges.view()
    ...
    # ---------------------------------------------------------------------------------------------
    # Method using Dask compute method to convert AggHistogram to a NumPy array
    # ---------------------------------------------------------------------------------------------

    def convert_agghistogram_to_numpy(self, dask_object):
        result = dask_object.compute(scheduler='threads', num_workers=21) 
        frequencies = result.to_numpy()[0]
        bin_edges = result.to_numpy()[1]
