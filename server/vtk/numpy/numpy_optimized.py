...
class HistogramApp:    
    def __init__(self):
        ...
        # self.np_data = np.random.normal(size=1_000)
    ...
    def compute_histogram_data_with_numpy(self, np_data, bins):  
        ...
        if self.data_min is None or self.data_max is None:
            self.data_min, self.data_max = self.compute_min_and_max_values_using_numpy(np_data)
        
        start_time_to_calculate_histogram = time.time()
        self.hist, self.bin_edges = np.histogram(np_data, bins=bins, range=(self.data_min, self.data_max))
    ...
    # -----------------------------------------------------------------------------------------------------------------------------
    # Method using NumPy to calculate the minimum and maximum values of data 
    # -----------------------------------------------------------------------------------------------------------------------------
    
    def compute_min_and_max_values_using_numpy(self, np_data):
        data_min = np.min(np_data)
        data_max = np.max(np_data)
        
        self.numpy_method_invoked += 1
        
        print("The number of times the compute_min_and_max_values_using_numpy method has been been called: ", self.numpy_method_invoked)
