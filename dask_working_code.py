...
class BaseOoDHistogram:                                                                                                                                                                       
    ...
    def __init__(self, name, port, csv_path = "", collection_path="",max_slices="",ood_column = None, study_column=None, series_column=None, image_column=None,node_id=None):                                                                                                                                                                                                                                       
        ...
        global SERVER_IP                                                                                                                                                                      
        SERVER_IP = ''   # add your server ip address                                                                                                                                                     
        global PORT                                                                                                                                                                           
        PORT ='8000' 

        #self.state.auth_key= "a2V5"
        ...
        self.state.image_base_url = os.path.join(settings.MEDIA_ROOT, max_slices) # Folder /media/lidc_pixConvImg
        self.state.collection_base_url = os.path.join(settings.MEDIA_ROOT, collection_path)  # Folder media/LIDC_Dataset

        
        self.state.files = "/home/cc/ood_test/neurobazaar/media/dicom_images" #Folder containing dicom downloaded images
        
        if csv_path and not ood_column:                                                                                                                                                      
        ...
        if self.csv_path:
            if self.collection_path and self.max_slices:
                ...
                if isinstance(self.data, np.ndarray):
                    ...
                    #print(self.data_dask)
                self.data_min, self.data_max = self._compute_min_max(self.data_dask)
    ...
    def update_plot(self, extra_lines=None):                                                                                                                                                       
        ...
        self.hist, self.bin_edges = self._compute_hist(self.data_dask, self.state.bins)
        if self.hist is None or self.bin_edges is None:
            return None    
        
        ax.set_xticks(np.arange(self.data_min, self.data_max+1,step=0.5)) 
        ax.set_xlim(left=self.data_min, right=self.data_max+1)
        ...
        if extra_lines:                                                                                                                                                                       
            for i,line in enumerate(extra_lines):                                                                                                                                             
                ...
                if i > 0:  # Draw an arrow only if there's a previous line                                                                                                                    
                    arrow = FancyArrowPatch((line, y_limits[1] * 1.2 ),                                                                                                                       
                                            (extra_lines[i - 1], y_limits[1] * 1.2),                                                                                                                                                                                                                                                          
                                            color=line_color,                                                                                                                                 
                                            arrowstyle='<->',                                                                                                                                 
    ...
    def display_data(self):     
        ...
        for i, item in enumerate(self.state.subset_items):                                                                                                                                    
            ...
            if start < end:                                                                                                                                                                   
                ...
                results = self.df[self.df[self.state.node_id].isin(node_basename)][[self.state.node_id, self.state.study_column, self.state.series_column, self.state.image_column, self.state.ood_column]]                                                                                                               
                                                                                                                                                                                                                                                                                        
                # List of tuples containing the Noduleids, Study, Series, SOP UIDs, and Ood scores                                                                                                                                                                                                                                                                                           
                mappings = [                                                                                                                                                                  
        ...
        if self.state.subset_items:                                                                                                                                                           
            ...
            if last_threshold<= max_value:                                                                                                                                                    
                ...
                # Map DICOM images to nodule IDs and Ood scores                                                                                                                               
                rem_results = self.df[self.df[self.state.node_id].isin(rem_node_basename)][[self.state.node_id, self.state.study_column, self.state.series_column,self.state.image_column, self.state.ood_column]]                                                                                                               
    ...
    def remove_subset(self, index):                                                                                                                                                           
        ...
        if 0 < index <= len(self.state.subset_items):                                                                                                                                         
            ...
            self.server.state.dirty("range_item")  
            self.server.state.dirty("subset_items")  
            self.html_figure.update(self.update_plot())                                                                                                                                        
    ...
    def render_ui_layout(self):                                                                                                                                                               
            ...
            with vuetify.VContainer(fluid=True, classes="d-flex flex-column flex-md-row",style="max-width: 100%; padding-bottom:10px"):                                                                                                                                                                                                                                   
                ...
                with vuetify.VCol(xs="12", sm="12", md="8", lg="9",xl="9"):                                                                                                                                                                                                                                                                          
                    ...
                    with vuetify.VRow(classes="justify-start"):                                                                                                                                          
                            ...
                            style="font-size: 18px;font-weight: bold;color: rgb(0, 71, 171); padding-top:10px")                                                                                                                                                                                                                                                                                      
                                                                                                                                                                                                                                                                                
                            with vuetify.VRow(classes="justify-start"):                                                                                                                                          
                                with vuetify.Template(v_for="(item, index) in data_items",key="index"): 
                                    with vuetify.VCol(xs="12", sm="12", md="8", lg="8",xl="9"):                                                                                                                                                                                                                                                                                
                                        with vuetify.VContainer(style="overflow-x: auto; white-space: nowrap; overflow-y: hidden; padding:15px;"):                                                                                                    
                                            vuetify.VIcon(                                                                                                                            
                                            "mdi-download",                                                                                                                   
                                            color="blue",                                                                                                                     
                                                    ...
                                                    with vuetify.Template(v_for="(dicom, imgIndex) in item.dicom_imgs", key="imgIndex"):                                                                                                                                                               
                                                        with vuetify.VCol(cols="auto", class_="d-inline-block", style="flex: 0 0 auto; padding: 5px; max-width:100%"):                                                                                                                                                               
                                                            ...
                                                            with vuetify.VCard(style="padding-top: 2px;padding-left:12px; max-height: 800px;"): 
                                                                ...
                                                                #with vuetify.Template(v_for="(entry, index) in item.download", key="index"):  
                                                                #    vuetify.VCardText("id:{{entry[0]}}|ood:{{entry[entry.length-1]}}",style="font-size: 16px; text-align: center; margin-top: 70px; margin-padding:5px",) 
        ...
        # Data route                                                                                                                                                                          
        with RouterViewLayout(self.server, "/data/:imgIndex/:dicom", style="max-width: 100%; max-height:10%; padding: 0; margin: 0;"):                                                                                                                                                                                                                                                           
            with vuetify.VRow(style="max-width: 100%; padding:0; margin: 0;"):                                                                                                                                                              
                #vuetify.VSpacer()                                                                                                                                                            
                with vuetify.VBtn("Take me back", click="$router.back()", style="margin: 15px;"):                                                                                                                                                                                                                                                                                           
                    vuetify.VIcon("mdi-arrow-left-bold", color="red", size=20)                                                                                                                                                                                                                                                                                         
                #with vuetify.VRow(style="display: flex; justify-content: center;padding-top:5px;"):
                with vuetify.VContainer(style="overflow-x: auto; white-space: nowrap; overflow-y: auto; padding-right: 15px; max-width: 100%;padding-top:10px"):  
                        with vuetify.VRow(style="display: flex; justify-content: space-between;"): 
                            ...
                            with vuetify.VCol(xs="12", sm="12", md="3", lg="5", xl="5"):  
                                with vuetify.VRow(style="display: flex; flex-wrap: nowrap; white-space: nowrap; align-items: flex-start; justify-content: center",):
                                    with vuetify.Template(v_for="(dicom, dicomIndex) in image_items.Original_Dicom", key="dicomIndex"):
                                        with vuetify.VCol(cols="auto", class_="d-inline-block", style="flex: 0 0 auto; padding-left: 20px; text-align:left; padding-left:10px"):
                                            vuetify.VSubheader("Selected DICOM:", style="font-size: 20px; font-weight: bold; color: rgb(0, 71, 171);")
                                            vuetify.VImg(
                                                src=("dicom", lambda name: f"{name}"),
                            ...
                            #with vuetify.VCol(xs="3", sm="12", md="3", lg="5", xl="5"):
                                with vuetify.VRow(style="display: flex; flex-wrap: nowrap; white-space: nowrap; align-items: flex-start; justify-content: center;"):    
                                    with vuetify.Template(v_for="(segment, segmentIndex) in image_items.Segmented_Nodule", key="segmentIndex"):
                                        with vuetify.VCol(cols="auto", class_="d-inline-block", style="flex: 0 0 auto; padding-right: 20px; text-align:right; padding-left:10px;"):
                                            vuetify.VSubheader("Selected Segmented Nodule:", style="font-size: 20px; font-weight: bold; color: rgb(0, 71, 171);")
                                            vuetify.VImg(
        ...
        with RouterViewLayout(self.server, "/compare/", style="width: 100%; padding: 0; margin: 0;"):   
            ...
            with vuetify.VRow(v_if="compare_images.length > 0 && compare_details.length > 0", style="display: flex; justify-content: center; align-items: center; padding-top: 30px;"):
                    ...
                    with vuetify.VCard(style="max-height: 700px;"):
                        with vuetify.Template(v_for="(item, index) in no_image", key="index"):
                            vuetify.VImg(
                                ...
                                lazy_src="http://picsum.photos/id/114/600/600",
...
if __name__ == "__main__":                                                                                                                                                                    
    server = BaseOoDHistogram("Ood Visualizer", 8090, "MaxSlices_wOoDScore.csv", "LIDC_Dataset", "lidc_pixConvImg", "Log_Loss_ALL","StudyInstanceUID","SeriesInstanceUid","imageSOP_UID","noduleID")                                                                                                                                                                                                                              
