...
class HistogramApp:
    ...
    def update_histogram(self):
        ...
        hist, bins = np.histogram(self.np_data, bins=self.server.state.bins, range=(self.data_min, self.data_max), density=True)
        x = (bins[:-1] + bins[1:]) / 2
        bin_width = bins[1] - bins[0]

        fig = go.Figure(data=[go.Bar(x=x, y=hist, width=bin_width)])
